...
INSERT INTO SOCIOS VALUES(4,'LUIS','MENDEZ',NULL,'10/05/2010',NULL,NULL,'10/05/2006','USUARIO',NULL,NULL,NULL,1); --> MENOR DE 18 AÑOS CON RESPONSABLE MAYOR DE 18 AÑOS
...
----------------------------- TRIGGER: COMPROBAR SI EL USUARIO HA PAGADOLA CUOTA ANTES DE FINAL DEL PRESENTE AÑO PARA RENOVAR--------

SELECT TO_CHAR(SYSDATE,'DDD') FROM DUAL; --> numeros de dias transcurridos desde el 1 de enero

CREATE OR REPLACE PROCEDURE VALIDAR_CUOTA (V_COD_SOCIO IN SOCIOS.COD_SOC%TYPE)

AS

BEGIN
DECLARE
V_DIAS_TRASCURRIDOS CHAR;
V_DIAS_A_PAGAR NUMBER;
V_PAGO NUMBER(24);
V_PAGO_FIN NUMBER;
BEGIN
    SELECT TO_CHAR(SYSDATE,'DDD') INTO V_DIAS_TRASCURRIDOS FROM DUAL;
    V_DIAS_A_PAGAR := 365-V_DIAS_TRASCURRIDOS; 
    SELECT IMPORTE_PAGO INTO V_PAGO FROM CUOTAS WHERE SOCIOS_COD_SOC= V_COD_SOCIO; 
    V_PAGO_FIN := (V_PAGO/365)*V_DIAS_TRASCURRIDOS;
        IF V_PAGO_FIN = 0 THEN
        UPDATE CUOTAS
        SET IMPORTE = 'PAGADO'
